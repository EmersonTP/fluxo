...
const TOOLS = [
  ...
  { name: "fluxo_update_task", description: "Atualiza campos de uma tarefa: nome, descrição, status, prioridade, prazo, responsáveis ou tags.", inputSchema: { type: "object", properties: { taskId: { type: "string" }, name: { type: "string" }, description: { type: "string", description: "Corpo/descrição da tarefa (texto ou markdown)" }, statusId: { type: "string" }, priority: { type: "string" }, dueDate: { type: "string" }, assigneeIds: { type: "array", items: { type: "string" } }, tagIds: { type: "array", items: { type: "string" } } }, required: ["taskId"] } },

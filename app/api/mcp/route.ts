...
const TOOLS = [
  ...
  { name: "fluxo_create_task", description: "Cria uma tarefa em uma lista. statusId opcional. Aceita description (texto/markdown do corpo da tarefa).", inputSchema: { type: "object", properties: { listId: { type: "string" }, name: { type: "string" }, description: { type: "string", description: "Corpo/descrição da tarefa (texto ou markdown)" }, statusId: { type: "string" }, priority: { type: "string", enum: ["urgent", "high", "normal", "low"] }, dueDate: { type: "string", description: "ISO date, ex: 2026-07-01" }, assigneeIds: { type: "array", items: { type: "string" } }, parentId: { type: "string" } }, required: ["listId", "name"] } },
  { name: "fluxo_update_task", description: "Atualiza campos de uma tarefa: nome, descrição, status, prioridade, prazo, responsáveis, tags ou lista (mover).", inputSchema: { type: "object", properties: { taskId: { type: "string" }, name: { type: "string" }, description: { type: "string", description: "Corpo/descrição da tarefa (texto ou markdown)" }, statusId: { type: "string" }, priority: { type: "string", enum: ["urgent", "high", "normal", "low"] }, dueDate: { type: "string", description: "ISO date; vazio limpa o prazo" }, assigneeIds: { type: "array", items: { type: "string" } }, tagIds: { type: "array", items: { type: "string" } }, listId: { type: "string", description: "Mover a tarefa para outra lista (ID)" } }, required: ["taskId"] } },
